...
function HallLayer:onCreate(parames)
    ...
    local csb = cc.CSLoader:createNode("NewHallLayer.csb")
    self:addChild(csb)
    ...
    if uiButton_mall~= nil then 
        Common:addTouchEventListener(uiButton_mall,function() 
            require("common.SceneMgr"):switchOperation(require("app.MyApp"):create():createView("NewXXMallLayer"))   --MallLayer
        end)      
    ...
    ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("hall/qinyouquan/qinyouquan.ExportJson")
    local armature=ccs.Armature:create("qinyouquan")
    armature:getAnimation():playWithIndex(0)
    uiButton_club:addChild(armature)
    ...
    ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("hall/chuangjianjiaru/chuangjianjiaru.ExportJson")
    local armature=ccs.Armature:create("chuangjianjiaru")
    armature:getAnimation():playWithIndex(0)
    uiButton_createFriendsRoom:addChild(armature)
    armature:setPosition(armature:getParent():getContentSize().width/2,armature:getParent():getContentSize().height/2)
    ...
    ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("hall/chuangjianjiaru/chuangjianjiaru.ExportJson")
    local armature=ccs.Armature:create("chuangjianjiaru")
    armature:getAnimation():play("jiaru")
    --armature:getAnimation():playWithIndex(1)
    uiButton_joinFriendsRoom:addChild(armature)
    armature:setPosition(armature:getParent():getContentSize().width/2,armature:getParent():getContentSize().height/2)
    ...
    --大厅人物点击音效
    local uiImage_character= ccui.Helper:seekWidgetByName(self.root,"Image_character")
    self.boss = 0
    Common:addTouchEventListener(uiImage_character,function() 
        self.boss = self.boss + 1 
        if self.boss <=1 or self.boss > 5 then
            self.boss = 1
        end
        require("common.Common"):playEffect(string.format("hall/boss%d.mp3",self.boss))
        uiImage_character:setEnabled(false)
        self:runAction(cc.Sequence:create(cc.DelayTime:create(3),cc.CallFunc:create(function(sender,event)  uiImage_character:setEnabled(true)  end)))
    end,true)
...
function HallLayer:updateUserInfo(event)
    ...
    local dwGold = Common:itemNumberToString(UserData.Bag:getBagPropCount(1009))    
