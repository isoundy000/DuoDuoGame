...
function KwxGameOver:updatePlayerStatics(root, statics,score)
	...
	local wkindID = GameCommon.tableConfig.wKindID  
	if wkindID == 47 or wkindID == 48  or wkindID == 89 then
		showEnd = {-1,1,3,5,6}
...
--用户统计
function KwxGameOver:updatePlayerStaticsNum(root, tScoreInfo,pBuffer)
	local listEnd = root:getChildByName('ListView_end')
	for i=1,pBuffer.dwDataCount do
		...
		Text_num:setColor(cc.c3b(177, 76, 15))
		-- local index = showEnd[i]
		Text_name:setString(EndListNum[i])
		if tScoreInfo.lScore[i] >= 0 then
			Text_num:setString(string.format("+%d",tScoreInfo.lScore[i]))
		else
			Text_num:setString(string.format("%d",tScoreInfo.lScore[i]))
		end
		listEnd:pushBackCustomItem(item)
